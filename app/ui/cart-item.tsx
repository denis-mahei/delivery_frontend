...
const CartItem = ( { id, name, price, quantity, imageUrl }: CartItem ) => {
	...
	return (
		<li className='group flex items-center gap-2.5'>
			<div className="flex items-center gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/80 p-3 sm:gap-4 w-full">
				...
				<div className="flex flex-col items-center gap-1">
					...
					<button
						...
						className="rounded-md bg-zinc-800 p-1 text-white transition hover:bg-zinc-700 mb-2"
					>
						<FaCaretDown/>
					</button>
				</div>
			</div>
				<button onClick={() => removeItem(id)} className='text-white group-hover:text-red-500 transition-all duration-200'>
					<IoTrashBinSharp/>
				</button>
		</li>
	);
